...
if [ -z "$term" ]; then
  if ! [ -p "play.pipe" ]; then
    mkfifo play.pipe;
    if [ -z "$buffer" ]; then
      nohup bash -c "printf \"\033[8;5;80t\"; aplay $alsaopt -t raw play.pipe || true; sleep 1; rm play.pipe" &
    else
      nohup bash -c "printf \"\033[8;5;80t\"; stdbuf -i$buffer aplay $alsaopt -t raw - <play.pipe || true; sleep 1; rm play.pipe" &
    fi;
  fi;
  if ! [ -p "rec.pipe" ]; then
    mkfifo rec.pipe;
    if [ -z "$buffer" ]; then
      nohup bash -c "printf \"\033[8;5;80t\"; arecord $alsaopt $recopt -t raw >>rec.pipe || true; rm rec.pipe" &
    else
      nohup bash -c "printf \"\033[8;5;80t\"; stdbuf -o$buffer arecord $alsaopt $recopt -t raw >>rec.pipe || true; rm rec.pipe" &
    fi;
  fi;
else
  if ! [ -p "play.pipe" ]; then
